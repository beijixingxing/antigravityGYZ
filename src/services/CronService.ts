        ...
        try {
            ...
            for (const cred of creds) {
                try {
                    ...
                    // 永久性错误：invalid_grant (Token 被撤销)、401、403
                    const isPermanent = errorCode === 'invalid_grant' || status === 401 || status === 403;
            ...
            for (const t of tokens) {
                ...
                try {
                    ...
                    if (is429 || is503 || is502 || is500 || is400 || isTimeout || isNetworkError) {
                        // 临时性错误，跳过这个凭证，下次再检查
                        skipped++;
                        console.log(`[CronService] Antigravity token ${t.id} (${t.email || 'unknown'}) skipped due to temporary error: ${msg.substring(0, 100)}`);
                        continue;
                    }
                    ...
                    if (!is403) {
                        // 其他未知错误，也跳过，避免误删
                        skipped++;
                        console.log(`[CronService] Antigravity token ${t.id} (${t.email || 'unknown'}) skipped due to unknown error: ${msg.substring(0, 100)}`);
                        continue;
                    }
