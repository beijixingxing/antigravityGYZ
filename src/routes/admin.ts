  ...
  fastify.post('/api/admin/health-check/cli', { preHandler: requireAdmin }, async (req, reply) => {
    ...
    for (const cred of allCredentials) {
      ...
      try {
        ...
        // 尝试调用 Cloud Code API 验证
        const testUrl = `https://cloudcode-pa.clients6.google.com/v1/projects/${cred.project_id || '-'}/locations/global/codeAssistants:generate`;
        const testBody = {
          assistant_id: 'aistudio-assistant',
          current_file: { contents: { code: '' } },
          contexts: [],
          user_query: { text: 'hi' }
        };
        ...
        // 凭证健康
        results.healthy++;
        console.log(`[Admin] 凭证 ${cred.id} 验证通过`);
