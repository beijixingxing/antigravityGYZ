...
const strictQuotaMode = ref(false);
...
const fetchSettings = async () => {
  try {
    ...
    strictQuotaMode.value = !!res.data.strict_quota_mode;
...
const toggleStrictQuotaMode = async () => {
  isLoading.value = true;
  message.value = '';
  try {
    const newValue = !strictQuotaMode.value;
    await api.post('/admin/settings', { strict_quota_mode: newValue });
    strictQuotaMode.value = newValue;
    message.value = newValue ? '已开启严格配额模式：V3 用户上传的 2.5 凭证将只增加 2.5 额度' : '已关闭严格配额模式：V3 用户所有凭证均享受 V3 增量';
  } catch (e) {
    message.value = '设置更新失败';
  } finally {
    isLoading.value = false;
    ...
    <div class="bg-white/5 backdrop-blur-xl border border-purple-500/30 rounded-2xl p-4 shadow-[0_0_15px_rgba(139,92,246,0.1)]">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div class="h-4 w-[1px] bg-white/10"></div>
                <div class="flex flex-col">
                    <h3 class="font-medium text-sm" :class="strictQuotaMode ? 'text-orange-400' : 'text-green-400'">
                        {{ strictQuotaMode ? '已开启：严格区分凭证类型' : '已关闭：V3 用户全员增益' }}
                    </h3>
                    <p class="text-xs text-[#A5B4FC] opacity-60">
                        {{ strictQuotaMode
                            ? 'V3 用户上传的 2.5 凭证只增加 2.5 额度，不享受 V3 增量'
                            : 'V3 用户上传的所有凭证均按 V3 增量计算 (默认)' }}
                    </p>
                </div>
            </div>

            <button
                @click="toggleStrictQuotaMode"
                :disabled="isLoading"
                class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none shrink-0"
                :class="strictQuotaMode ? 'bg-gradient-to-r from-orange-400 to-red-500' : 'bg-white/10'"
            >
                <span class="sr-only">切换严格配额模式</span>
                <span
                class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform shadow"
                :class="strictQuotaMode ? 'translate-x-6' : 'translate-x-1'"
                />
            </button>
        </div>
    </div>
